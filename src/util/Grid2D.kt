...
class Grid2D<T>(input: String, elemDelimiter: String) {
	...
	fun copyWithModifications(modifyPosition: List<Position<T>>, newValue: List<T>): Grid2D<T> {
		val newArray = array.map { it.toMutableList() }.toMutableList()
		for (i in modifyPosition.indices)
			newArray[modifyPosition[i].row][modifyPosition[i].col] = newValue[i]
		return Grid2D(newArray)
	}
}